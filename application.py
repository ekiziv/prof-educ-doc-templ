    ...
    if add_new:  # Only show input if checkbox is checked
        ...
        if st.button("Добавить программу"):
            if new_profession:
                ...
                return f'{code_int} «{new_profession}»'  # Return the new profession if added
            else:
                st.warning("Программа уже существует или не введена.")
    if selected_item: 
        selected_profession_code = all_professions[selected_item]
        selected_profession_code_str = ', '.join(str(code) for code in selected_profession_code)
        return f'{selected_profession_code_str} «{selected_item}»'  # Return selected_item otherwise
    return None, None
...
company = st.text_input("Предприятие", 'заявление', placeholder="Наименование предприятия или 'заявление'")
...
for line in student_names:
    if line:
        ...
        certificate_number, _, _, name  = filtered_items  # Split each line by tab
        student_data.append({
            NAME_KEY: name,
...
if show_documents:
    ...
    if student_profession and teacher_name and beginning_date and end_date and beginning_number and end_number: 
        document_tabs = st.tabs(["Приказ о начале", "Приказ об окончании", "Протокол", "Свидетельство", "Свидетельство тракторов"])
